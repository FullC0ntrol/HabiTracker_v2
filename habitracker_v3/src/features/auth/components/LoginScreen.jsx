...
/**
 * Ekran logowania – użytkownik podaje nazwę konta (username),
 * następnie przechodzi do ekranu PIN.
 */
export default function LoginScreen({ onComplete }) {
  const { setUsername } = useAuth(); // zapisuje login do globalnego kontekstu
  ...
  const handleSubmit = (e) => {
    ...
    // 🧠 zapisz login do AuthContext (i opcjonalnie localStorage)
    setUsername(trimmed);
    localStorage.setItem("username", trimmed);

    // 🔄 przejdź dalej (do ekranu PIN)
    onComplete?.();
  };
    ...
    <div className="min-h-screen w-full bg-mesh flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-scale-in">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 text-sm">Zaloguj się, by śledzić swoje nawyki</p>
        </div>
