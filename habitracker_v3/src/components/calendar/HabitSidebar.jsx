// client/src/components/calendar/HabitSidebar.jsx
...
export function HabitSidebar({
  ...
  habits = [],
  todayCounts = {},
  onIncrement, // (habitId, delta)
}) {
  // Lokalne "dzisiaj" w strefie Europe/Warsaw jako YYYY-MM-DD
  const nowISO = new Date().toLocaleDateString("en-CA", { timeZone: "Europe/Warsaw" });
  const isToday = todayISO === nowISO;

  // sortowanie: najpierw nieukończone, potem malejąco po brakującym progresie
  const sorted = [...habits].sort((a, b) => {
    ...
    const aIncomplete = aMissing > 0;
    const bIncomplete = bMissing > 0;
        ...
        {!isToday && (
          <div className="mx-4 mt-4 mb-0 rounded-lg border border-white/10 bg-white/5 text-white/80 px-3 py-2 flex items-center gap-2">
            ...
            <span className="text-sm">
              Możesz odhaczać tylko <span className="font-semibold">dzisiaj</span> ({nowISO}). Ten dzień jest zablokowany.
            </span>
          </div>
        )}
          ...
          {sorted.length === 0 ? (
            ...
            <ul className="space-y-3">
              {sorted.map((h) => {
                ...
                const renderSingleCheck = () => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={label}
                      title={!isToday ? "Odhaczanie niedostępne dla tego dnia" : label}
                    >
                      <span
                ...
                // target > 1: kratki do odhaczania
                const renderMultiChecks = () => {
                  const tooMany = target > 12;
                  if (tooMany) {
                    ...
                    return (
                      <div className="w-full">
                        ...
                        <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                          <div
                            className={`h-full transition-all ${isDone ? "bg-emerald-500" : "bg-cyan-500"}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                      </div>
                        ...
                        {Array.from({ length: target }).map((_, i) => {
                          ...
                          return (
                            <button
                              ...
                              title={
                                !isToday
                                  ? "Odhaczanie niedostępne dla tego dnia"
                                  : checked
                                  ? `Odznacz (${i + 1}/${target})`
                                  : `Odhacz (${i + 1}/${target})`
                              }
                            >
                              {checked ? <Check className="w-4 h-4 text-white" /> : <span className="w-2 h-2 rounded-sm bg-white/30" />}
                            </button>
                          );
                        })}
                      </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className={`font-semibold truncate ${isDone ? "text-emerald-300" : "text-white"}`}>
                          {h.name}
                        </div>
                      ...
                      <div className="shrink-0">
                        {target > 1 ? renderMultiChecks() : renderSingleCheck()}
                      </div>
                    </div>
                  </li>
                );
